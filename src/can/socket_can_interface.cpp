...
: socket_fd_(-1), interface_name_(interface_name), logger_(rclcpp::get_logger("SocketCanInterface"))
{
  if (!InitializeSocketCAN()) {
    RCLCPP_ERROR(logger_, "Failed to initialize SocketCAN interface: %s", interface_name_.c_str());
  }
}
...
bool SocketCanInterface::InitializeSocketCAN()
{
  ...
  if (ioctl(socket_fd_, SIOCGIFINDEX, &ifr) < 0) {
    RCLCPP_ERROR(logger_, "Failed to get interface index for %s", interface_name_.c_str());
    CloseSocketCAN();
    return false;
  }
  ...
  if (bind(socket_fd_, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    RCLCPP_ERROR(logger_, "Failed to bind CAN socket to interface %s", interface_name_.c_str());
    CloseSocketCAN();
    return false;
  }

  RCLCPP_INFO(logger_, "SocketCAN interface %s initialized", interface_name_.c_str());
  return true;
}
